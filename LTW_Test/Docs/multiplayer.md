...
the whole world. An order is not applied where it is given: the relay seals it into a TURN a little
...
| D34 | **The relay SPEAKS FOR a player who has left**, rather than peers agreeing to stop waiting. | 2026-09-05 | An empty word every turn, for the rest of the match. The obvious alternative - an order saying "stop waiting from turn T" - deadlocks: a peer waiting for the leaver is stalled, its clock frozen, so it can never reach T. Built it that way first and measured the survivor stopping at 227 turns. **The order that unblocks the turn stream cannot ride the turn stream.** |
...
| D37 | ~~**The input delay allows for THIS MACHINE's frame-time overrun as well as the network's jitter.**~~ **Largely superseded by D40.** The term it added fed `_wire_budget_ms`, which is gone; `local_jitter_ms` survives only as a session-log reading, and what it was compensating for - a peer's own ticks running late - is now handled by that peer alone rather than charged to everybody. Kept because the READING is still the thing that separates "the wire is slow" from "this machine is". | 2026-09-06 | `adaptive_local_jitter`. The flat `jitter_margin_ms` was zeroed on HEADLESS paired runs, which have no renderer and therefore almost no frame-time variance - an honest measurement of the network term that silently zeroed a term only a drawing client has. Playtest 1 is the case: nothing was wrong with the wire, and a peer's words were late because its own ticks were. Measured live as a 90th percentile over three seconds and capped, so a machine keeping up pays nothing. **Not yet verified against a real link** - the percentile and the cap are chosen from the shape of the problem, and the session log's health line now carries the number that will say whether they are right. |
...
| D13 | **No reconnect. Out is out.** A player who disconnects is gone for that match. | 2026-08-21 | User's call. Simplifies a lot — see §11.1. Does not rule out a short grace period before declaring someone gone, which is a different thing. Before the go signal, a player whose link to the match process breaks may claim their seat again within D26's hold (D44, 2026-09-25); from the go signal on, this row holds unchanged. |
...
| `Scripts/Game/ShaderWarmup.gd` | Draws everything a match can show once, behind a black screen, before this machine says it is ready - the Compatibility renderer compiles a shader on its first draw and nothing else can ask it to. Holds the world still by name while it runs. |
...
PlayerState.steal_life_from(victim) -> int     # how many lives moved, 0 for none
Replication.report_leak(thief, victim, lives) # authority only; raises leak_reported
Replication.leak_reported                     # signal, on every machine drawing a world
```

`report_leak` is the one EVENT on the replication service, which is otherwise pure state.
It is there because two snapshots of a player's lives say a number changed and cannot say
who took it, and because an `@rpc` has to live on an autoload anyway. It is reliable, unlike
the snapshot: a lost snapshot is replaced 50 ms later, a lost message is simply never shown.
Who is allowed to READ a leak is the HUD's question - see `LeakLog`, which shows only the
lines the local player is at one end of.
...
> **THE MECHANISM BELOW WAS DELETED ON 2026-09-09 AND THE SECTION IS KEPT AS THE RECORD OF WHY.**
> A peer no longer closes turns or books an order into one; it sends the order bare at press time
> and the relay decides which turn it lands in. So `delay_turns`, `_wire_budget_ms`,
> `announce_one_way` and the whole measured-from-the-connection chain are gone.
>
> **What replaced it, and the arithmetic that still applies:** a player's wait is now the trip to
> the relay, plus the seal coming back, plus however far behind the relay that peer is playing -
> its jitter buffer, which is local and costs only its owner. Measured on two physical machines:
> 105 ms on a healthy desktop and 300 ms on a struggling laptop, where the difference was almost
> entirely the laptop's buffer. Catch-up (`_pace_engine`) exists to repay that buffer rather than
> let it stand for the rest of the match.
>
> **What survives unchanged is the REASONING**, which is why this is kept: the delay is a LIVENESS
> parameter and never a correctness one, so peers may hold different values and change them
> mid-match with no risk of divergence. That is what makes the catch-up servo safe, and it is the
> same argument written below for a mechanism that no longer exists.
>
> The feedback and prediction half of this section (D17) is untouched and still current.
