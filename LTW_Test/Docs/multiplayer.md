...
| **Redundancy under real packet loss** | Every turn word is echoed unreliably beside the reliable one. Proven to CARRY the data by running a match with the reliable path disabled entirely; never tested against actual loss, because loopback drops nothing. | Wants a link conditioner. |
...
| **A floor on how far ahead a peer books** | `delay_turns()` is decided locally, so a modified client could choose the minimum on a bad link and take a reaction-time edge while stalling everybody else - indistinguishable from a bad connection. | The relay could enforce it. Nothing to do until somebody has a reason to cheat. |
