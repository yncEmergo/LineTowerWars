class_name MoveAnimation2D
extends Node

# This can be used if the animator is only used for one specific animation
@export_group("Single Animiation Settings")
@export var single_animation_start_pos: Vector2
@export var single_animation_target_pos: Vector2
...
@export var interpolate_target_position: Vector2
...
var interpolate_start_position: Vector2

var target: Node
var tween: Tween

var has_interpolated: bool = false
var wiggle_rest_position: Vector2
...
func start_interpolate_manual(from: Vector2, to: Vector2, duration: float) -> void:
	if add_debugging:
	...
	var dist_from_to := from.distance_to(to)
	var dist_current_to_from := current.distance_to(from)
	...
	var duration_to_from := duration * ratio
...
func side_wiggle(max_x_offset: float, duration_per_movement: float, total_duration: float) -> void:
	...
	while elapsed < total_duration:
		...
		dir *= -1.0
		var target_pos := wiggle_rest_position + Vector2(max_x_offset * dir, 0.0)
