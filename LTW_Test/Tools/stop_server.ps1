...
# **`--match-server` has to be named separately, and the reason is a substring
# that is not one** (D44). `*--server*` does NOT match `--match-server`: the
# pattern needs two consecutive hyphens, and there the `server` is preceded by
# one (`h-server`). So a lobby was stopped while its children played on, leaving
# orphans holding ports the next lobby then failed to bind - and the comment that
# used to sit here asserted the opposite, confidently, having never been run.
$servers = @(Get-CimInstance Win32_Process -Filter "Name like '%odot%'" -ErrorAction SilentlyContinue |
    Where-Object { $_.CommandLine -and $_.CommandLine -like "*$projectName*" -and
                   ($_.CommandLine -like "*--server*" -or $_.CommandLine -like "*--match-server*") })
